<template>
    <v-container>
        <v-row class="d-flex justify-content-betwenn mt-5">
            <v-col>
                <v-form>
                    <v-row>
                        <v-col cols="auto">
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-table>
                            <thead>
                                <tr>
                                    <th>제품사진</th><th>제품명</th><th>가격</th><th>제고수량</th><th>주문수량</th><th>Action</th>
                                </tr>
                            </thead>
                            <tbody>
...
export default{
    ...
    methods:{
        ...
        scrollPaging(){
            // 현재 화면 높이 + 스크롤로 이동한 거리 > 전체 화면 높이 - n이 성립하면 추가 데이터 로드
        ...
        addCart(){
            ...
            const data = this.productList.filter(p=>p.selected&& p.productCount>0).map(p=>({productId:p.id, productName:p.name, productCount:p.productCount, productPrice:p.price}));
